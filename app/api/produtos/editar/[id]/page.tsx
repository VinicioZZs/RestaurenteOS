// app/configuracao/produtos/editar/[id]/page.tsx - VERSÃO COMPLETA CORRIGIDA
'use client';
...
export default function EditarProdutoPage() {
  ...
  const carregarDados = async () => {
    try {
      ...
      // Carregar categorias e adicionais
      const [resCategorias, resAdicionais] = await Promise.all([
        fetch('/api/categorias?ativas=true'),
        fetch('/api/adicionais?ativos=true')
      ]);
      ...
      if (dataAdicionais.success) setAdicionais(dataAdicionais.data);
      ...
      if (dataProduto.success) {
        ...
        if (Array.isArray(produto.tags)) {
          tagsString = produto.tags.join(', ');
        } else if (typeof produto.tags === 'string') {
          tagsString = produto.tags;
        }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center">
              <div className="relative">
                ...
                >
                  <span
                    className={`block h-6 w-6 mt-1 ml-1 rounded-full bg-white transition-transform ${
